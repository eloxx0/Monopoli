...
public:
    ...
    Player *get_propriety()
    { // ritorna puntatore a player
        return player;
    }
    ...
    void set_belongings(int x)
    { // check operazione
        if (x >= 0 && x < 4)
            belongings = x;
        else
            std::cout << "Impossibile effettuare l'operazione" << std::endl;
    }

    int get_belongings() const
    { // getter possedimenti per il giocatore rispetto ad una casella
        return belongings;
    }
